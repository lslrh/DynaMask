    ...
    roi_head=dict(
        ...
        mask_head=dict(
            type='DynaMaskHead',
                 ...
                 loss_cfg=dict(
                    type='DynaCrossEntropyLoss',
                    stage_instance_loss_weight=[0.5, 0.75, 0.75, 1.0],
                    stage_detail_loss_weight=[0.5, 0.5, 0.5, 0.5],
                    detail_loss_weight=DETAIL_LOSS_WEIGHT,
                    cb_loss_weight = CB_LOSS_WEIGHT,
                    boundary_width=2,
                    start_stage=4))))
...
data_root = './data/coco' # change this to your own path
data = dict(
    samples_per_gpu=4,
    workers_per_gpu=4,
    train=dict(
...
# optimizer_config = dict(type='OptimizerHook_', grad_clip=dict(max_norm=35, norm_type=2))
total_epochs = 12
lr_config = dict(
    policy='step',
    warmup='linear',
