...
def mask_cross_entropy(pred,
    ...
    pred_slice = pred[inds, label.int()].squeeze(1)
...
@LOSSES.register_module()
class DynaCrossEntropyLoss(nn.Module):
    ...
    def __init__(self,
                 ...
                 boundary_width=2,
                 start_stage=1):
        super(DynaCrossEntropyLoss, self).__init__()
    ...
    def forward(self, stage_instance_preds, stage_detail_preds, stage_instance_targets, mask_labels):
        loss_mask_set = []
        loss_detail_set = []
        for idx in range(len(stage_instance_preds)):
            instance_pred, instance_target = stage_instance_preds[idx].squeeze(1), stage_instance_targets[idx]
            ...
            if idx <= self.start_stage:
                ...
                loss_detail = mask_cross_entropy(detail_pred, detail_target, label=mask_labels[:,idx].view(-1,1,1))\
