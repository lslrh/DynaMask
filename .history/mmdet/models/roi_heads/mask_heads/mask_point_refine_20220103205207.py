...
class SFMStage(nn.Module):
    ...
    def __init__(self,
        ...
        fc_in_channels = fc_in_channels*2 
        for _ in range(num_fcs):
            ...
            fc_in_channels = fc_in_channels*2 if self.coarse_pred_each_layer else 0
    ...
    def forward(self, instance_feats, semantic_feat, rois, roi_labels, cfg):
        ...
        point_indices, rel_roi_points = self.get_roi_rel_points_train(detail_masks, cfg)
        ...
        mask_point_feats_out = self.fc_logits(mask_point_feats)

        refined_instance_feats = coarse_feats.scatter_(2, point_indices, mask_point_feats_out)
        refined_instance_feats = refined_instance_feats.view(num_rois, channels, mask_height, mask_width)
        fused_instance_feats = self.relu(self.fuse_transform_out(refined_instance_feats))
        fused_instance_feats = self.relu(self.upsample(fused_instance_feats))
        return instance_preds, detail_preds, fused_instance_feats
