...
class MultiBranchFusion(nn.Module):
    ...
    def __init__(self, feat_dim, dilations=[1, 3, 5]):
        ...
        self.SEBlock = SEBlock(feat_dim*3)
        self.merge_conv = ConvModule(feat_dim*3, feat_dim, kernel_size=1, act_cfg=None)

    def forward(self, x):
        feat_1 = self.dilation_conv_1(x)
        feat_2 = self.dilation_conv_2(x)
        feat_3 = self.dilation_conv_3(x)
        out_feat = torch.cat((feat_1, feat_2, feat_3), 1)
        out_feat = self.SEBlock(out_feat)
        out_feat = self.merge_conv(out_feat)
        return out_feat
...
class SFMStage(nn.Module):
    ...
    def __init__(self,
        ...
        self.fuse_conv = nn.ModuleList([
            ...
            DCN(instance_in_channel, instance_in_channel, kernel_size=(3, 3), stride=(1, 1), padding=1, deform_groups=2, im2col_step=100)])
    ...
    def forward(self, instance_feats, semantic_feat, rois, roi_labels, upsample=True):
        ...
        concat_tensors.append(instance_preds.sigmoid())
        concat_tensors.append(detail_preds.sigmoid())
        ...
        fused_feats = torch.cat([fused_feats, instance_preds.sigmoid(), detail_preds.sigmoid()], dim=1)
        fused_feats = self.relu(self.upsample(fused_feats)) if upsample else fused_feats

        return instance_preds, detail_preds, fused_feats


@HEADS.register_module()
class DynaMaskHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_cfg=dict(
                    ...
                    boundary_width=2,
                    start_stage=1)):
        super(DynaMaskHead, self).__init__()
        ...
        for idx, out_size in enumerate(self.stage_sup_size[:-1]):
            ...
            new_stage = SFMStage(
                ...
                semantic_out_stride=semantic_out_stride[-1],
    ...
    def init_weights(self):
        for m in [self.final_instance_logits, self.final_detail_logits]:
            nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            nn.init.constant_(m.bias, 0)

    def forward(self, instance_feats, semantic_feats, rois, roi_labels):
        ...
        for idx, stage in enumerate(self.stages):
            ...
            instance_preds, detail_preds, instance_feats = stage(instance_feats, semantic_feats[-idx-3], rois, roi_labels, upsample_flag)
        ...
        if not self.pre_upsample_last_stage:
            ...
            stage_instance_preds.append(instance_preds)
            detail_preds = F.interpolate(detail_preds, scale_factor=2, mode='bilinear', align_corners=True)
            stage_detail_preds.append(detail_preds)
    ...
    def loss(self, stage_instance_preds, stage_detail_preds, stage_instance_targets):
        loss_instance = self.loss_func(
            stage_instance_preds, stage_detail_preds, stage_instance_targets)
