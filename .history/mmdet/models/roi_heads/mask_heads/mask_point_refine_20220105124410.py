...
class SFMStage(nn.Module):
    ...
    def forward(self, instance_feats, semantic_feat, rois, roi_labels, cfg):
        ...
        A = torch.sigmoid(detail_preds).detach().cpu().numpy()
        out = Image.fromarray(A[0][0]*255)
        out = out.convert('L')
        out.save('detail.jpg')
        pdb.set_trace()

        A = torch.sigmoid(detail_masks).detach().cpu().numpy()
        out = Image.fromarray(A[0][0]*255)
        out = out.convert('L')
        out.save('detail.jpg')
        pdb.set_trace()
        ...
        coarse_feats_ = coarse_feats.clone()
        point_indices = point_indices.unsqueeze(1).expand(-1, channels, -1)
        coarse_point_feats = torch.gather(coarse_feats, 2, point_indices.detach())
        ...
        refined_instance_feats = coarse_feats_.scatter_(2, point_indices.detach(), mask_point_feats)
