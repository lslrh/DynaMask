...
class SFMStage(nn.Module):
    ...
    def forward(self, instance_feats, semantic_feat, rois, roi_labels, cfg):
        ...
        instance_logits = self.instance_logits(instance_feats)
        instance_preds = instance_logits[torch.arange(len(rois)), roi_labels][:, None]
        detail_logits = self.detail_logits(instance_feats)
        ...
        fine_grained_point_feats = self._get_fine_grained_point_feats(semantic_feat, rois,
                             rel_roi_points)
        coarse_feats = instance_feats.view(num_rois, channels, -1).clone()
        # pdb.set_trace()
        ...
        point_indices_2 = point_indices.unsqueeze(1).expand(-1, self.num_classes, -1)
        coarse_point_instance_feats = torch.gather(instance_logits, 2, point_indices_2.detach())
        coarse_point_detail_feats = torch.gather(detail_logits, 2, point_indices_2.detach())
        ...
        point_indices_1 = point_indices.unsqueeze(1).expand(-1, channels, -1)
        refined_instance_feats = coarse_feats.scatter_(2, point_indices_1.detach(), mask_point_feats)
