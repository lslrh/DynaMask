...
from mmcv.ops import DeformConv2dPack as DCN_P
...
class SFMStage(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_offset = nn.Conv2d(
            semantic_out_channel*2,
            18,
            3,
            padding=1)
        self.dconv_featureflow = DCN(
            semantic_out_channel,
            semantic_out_channel,
            3,
            padding=1)        
        ...
        self.fuse_conv = nn.ModuleList([
            nn.Conv2d(fuse_in_channel, instance_in_channel, 1),
            DCN_P(instance_in_channel, instance_in_channel, kernel_size=(3, 3), stride=(1, 1), padding=1, deform_groups=2)])

        # self.fuse_transform_out = nn.Conv2d(instance_in_channel, instance_out_channel - 2, 1)
        self.fuse_transform_out = MaskedConv2d(instance_in_channel, instance_out_channel - 2, 1)
    ...
    def _init_weights(self):
        for m in [self.semantic_transform_in, self.detail_logits, self.instance_logits, self.fuse_transform_out, self.conv_offset]:
            nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            nn.init.constant_(m.bias, 0)
        
        nn.init.kaiming_normal_(self.dconv_featureflow.weight, mode='fan_out', nonlinearity='relu')
    ...
    def forward(self, instance_feats, semantic_feat, semantic_pred, rois, roi_labels):
        ...
        offset = self.conv_offset(torch.cat([instance_feats, ins_semantic_feats], dim=1))
        semantic_feats_align = self.relu(self.dconv_featureflow(ins_semantic_feats, offset))
        concat_tensors.append(semantic_feats_align)
