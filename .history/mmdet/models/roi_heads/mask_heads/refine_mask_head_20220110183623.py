...
class SFMStage(nn.Module):
    ...
    def _init_weights(self):
        for m in [self.semantic_transform_in, self.semantic_transform_out, self.instance_logits, self.detail_logits, self.fuse_transform_out]:
...
class RefineMaskHead(nn.Module):
    ...
    def forward(self, instance_feats, semantic_feats, rois, roi_labels):
        ...
        stage_detail_preds = []
        for idx, stage in self.stages:
            upsample_flag = self.pre_upsample_last_stage or idx < len(self.stages) - 1
            instance_preds, detail_preds, instance_feats = stage(instance_feats, semantic_feats[idx], rois, roi_labels, upsample_flag)
    ...
    def loss(self, stage_instance_preds, stage_detail_preds, semantic_pred, mask_labels, stage_instance_targets, semantic_target):

        loss_instance, loss_semantic = self.loss_func(
            stage_instance_preds, stage_detail_preds, semantic_pred, mask_labels, stage_instance_targets, semantic_target)
