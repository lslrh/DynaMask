...
class SFMStage(nn.Module):
    ...
    def forward(self, instance_feats, semantic_feat, rois, roi_labels, cfg):
        ...
        point_indices_ = point_indices.unsqueeze(1).expand(-1, self.num_classes, -1)
        coarse_point_instance_feats = torch.gather(instance_logits, 2, point_indices_.detach())
        coarse_point_detail_feats = torch.gather(detail_logits, 2, point_indices_.detach())
