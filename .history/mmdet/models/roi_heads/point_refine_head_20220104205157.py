...
class PointRefineRoIHead(StandardRoIHead):
    ...
    def _mask_forward_train(self, x, sampling_results, bbox_feats, gt_bboxes, gt_masks, gt_labels, img_metas):
        ...
        mask_results = self._mask_forward(x, pos_rois, torch.cat(pos_labels), self.train_cfg)
    ...
    def _mask_forward(self, x, rois, roi_labels, cfg):
        """Mask head forward function used in both training and testing."""

        ins_feats = self.mask_roi_extractor(x[:self.mask_roi_extractor.num_inputs], rois)
        stage_instance_preds, stage_detail_preds, semantic_pred = self.mask_head(ins_feats, x[0], rois, roi_labels, self.train_cfg)
    ...
    def simple_test_mask(self, x, img_metas, det_bboxes, det_labels, rescale=False):
        ...
        if det_bboxes.shape[0] == 0:
            ...
            for i in range(0, det_labels.shape[0], interval):
                mask_results = self._mask_forward(x, mask_rois[i: i + interval], det_labels[i: i + interval], self.test_cfg)
