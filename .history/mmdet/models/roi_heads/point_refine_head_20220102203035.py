...
class PointRefineRoIHead(StandardRoIHead):
    ...
    def _mask_forward_train(self, x, sampling_results, bbox_feats, gt_bboxes, gt_masks, gt_labels, img_metas):
        ...
        loss_mask, loss_semantic = self.mask_head.loss(
            ...
            semantic_target)
        loss_flops = {}
        loss_flops['loss_flops'] = self.train_cfg.Lambda * torch.clamp\
            ((torch.sum(mask_labels*torch.Tensor(self.train_cfg.flops).cuda())/len(mask_labels)-1.0)/(self.train_cfg.flops[-1]-self.train_cfg.flops[0]), min=0)
        mask_results.update(loss_mask=loss_mask, loss_semantic=loss_semantic, loss_flops=loss_flops)
    ...
    def _mask_forward(self, x, rois, roi_labels):
        ...
        stage_instance_preds, stage_detail_preds, semantic_pred = self.mask_head(ins_feats, x[0], rois, roi_labels)
        return dict(stage_instance_preds=stage_instance_preds, \
            stage_detail_preds=stage_detail_preds, semantic_pred=semantic_pred)
